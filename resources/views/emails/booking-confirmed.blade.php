...
<head>
    ...
    <style>
        ...
        .header { background: #059669; padding: 30px; text-align: center; color: white; }
        .content { padding: 30px; }
        .box { background: #f1f5f9; border-radius: 12px; padding: 20px; margin: 20px 0; }
        .btn { display: inline-block; background: #1e40af; color: white !important; padding: 12px 28px; border-radius: 10px; text-decoration: none; font-weight: 600; margin-top: 15px; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Great news! Your tutoring appointment with <strong>{{ $booking->tutor->name }}</strong> has been officially confirmed.</p>
            <div class="box">
                <p style="margin: 0 0 8px;"><strong>Booking Reference:</strong> #{{ $booking->booking_code }}</p>
                <p style="margin: 0 0 8px;"><strong>Tutor:</strong> {{ $booking->tutor->name }}</p>
                <p style="margin: 0 0 8px;"><strong>Subject:</strong> {{ $booking->subject?->name ?? 'General Tutoring' }}</p>
                <p style="margin: 0 0 8px;"><strong>Date:</strong> {{ $booking->booking_date->format('l, M d, Y') }}</p>
                ...
                <p style="margin: 0;"><strong>Mode:</strong> {{ ucfirst($booking->mode) }}</p>
            </div>
            <p>You can message your tutor directly from your student portal or access any shared study materials before class.</p>
            <a href="{{ url('/student/bookings') }}" class="btn">View Session Details</a>
        </div>
        <div class="footer">
